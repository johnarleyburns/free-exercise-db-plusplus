...
# 38. v1.11 — Python/Swift semantic parity (complete)

## Objective

Complete the Python reference audit and bring the native Swift engine to
semantic parity with Python. Kotlin and R engine parity are intentionally
future milestones.
...
Expected results are generated by the versioned Python reference and committed
for review. The remediation work tracked in HANDOFF.md must expand the
currently incomplete evaluation, history, progression, generation, and
adaptation matrices before v1.11 acceptance is considered complete.
...
# 43. Swift v1.11 objective (complete, pending remediation evidence)
...
As part of the completed v1.11 engine surface and retained in v1.12:
...
# 52. Kotlin future scope

Kotlin is not a v1.11 or v1.12 implementation target. Preserve existing
buildable functionality and schedule the full native/JVM/Android engine port
for v1.13.

Do not add Kotlin generation, state, adaptation, parity fixtures, or packaging
capability during the current release sequence.
...
Adaptive coaching parity is deferred to v1.13.
...
For v1.11 and v1.12, Python and Swift generation must produce semantically
identical plans for shared reference policies. Kotlin parity is v1.13 work.
...
- existing Kotlin tests remain green without expanding capability;
- existing R tests remain green without expanding capability;
- package docs are accurate;
- parity CI is green.

---

# 64. v1.12 — Swift SPM production hardening

## Objective

Make the completed native Swift engine easy to consume from real applications.

Python remains the reference package. Kotlin and R are future milestones and
are not expanded by v1.12.

---

# 65. v1.12 Swift package goal

The Swift package must be installable through normal SPM workflows:

```text
Swift Package Manager
```

Installability, documentation, distribution, resource loading, and drop-in
application readiness are mandatory. Python wheel acceptance remains a v1.11
reference-oracle gate, not new v1.12 engine work.
...
The package should expose structures that an application adapter can create or
consume, but must not know about or depend on an LLM.
...
# 74. Kotlin v1.13 final capability

Kotlin should support end-to-end app usage for the declared core workflow in
v1.13, not v1.12:
...
If any feature remains unsupported in v1.13, capability docs must say so
before that release.

Goal is full parity with Swift/Python for training-domain operations by v1.13.

---

# 75. Kotlin Android usability (v1.13)
...
# 76. Python v1.12 maintenance capability
...
# 82. Shared fixtures and Swift parity by v1.12

The v1.11 remediation must complete these mandatory fixture families; v1.12
must keep them green and add Swift package-consumer coverage:
...
A release cannot claim Python/Swift parity if the fixture suite is red. Existing
Kotlin/R checks remain in the repository suite but do not become new v1.12
parity gates.
...
# 89. v1.12 Swift final golden acceptance test
...
Python and Swift must produce semantically equivalent:
...
Existing R artifact checks remain green; new R parity is v1.14 work.

---

# 90. v1.12 Swift history-aware golden test
...
Python and Swift should produce semantically equivalent:
...
R remains outside the v1.12 implementation scope.
...
# 92. v1.12 Swift release gate

v1.12 is complete only when:

1. v1.11 Python/Swift parity remediation is complete;
2. all Python/Swift shared fixtures are green;
3. WorkoutIntent resolution and the flagship scenario are deterministic;
4. Swift SPM installs and resolves cleanly from an external consumer;
5. Swift bundled and injected resources work outside the repository directory;
6. Swift public API is typed, documented, ergonomic, and compatibility-tested;
7. Swift supports generation, evaluation, state derivation, and adaptation;
8. Swift exposes database, relationship, schema, policy, and package provenance;
9. Swift semantic results and typed errors follow the documented contract;
10. Swift value models are truthfully Sendable and concurrent reads are safe;
11. initialization, evaluation, generation, state, and adaptation benchmarks
    are recorded for representative mobile-scale fixtures;
12. package/resource/binary-size measurements are recorded;
13. executable command-line and application-service examples compile and run;
14. all core operations work offline without Python, subprocesses, network, or
    LLM/FoundationModels dependencies;
15. existing Kotlin and R tests remain green without new parity scope;
16. documentation and capability matrices contain no stale schedule claims;
17. full repository tests, exact-SHA CI, release workflow, and post-release
    packaged-artifact smoke are green.
...
Only after v1.10 semantics are stable should v1.11 implement the complete
Python/Swift native parity milestone. Kotlin and R remain later milestones.
