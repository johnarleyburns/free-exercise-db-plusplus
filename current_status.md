...
## Part AU adaptation-flow evidence

The Swift application surface exposes generation from new intent separately
from `adaptPlan(profile:target:currentPlan:history:asOf:...)`. Hosts therefore
choose generation or adaptation explicitly; no automatic mode switching or
...
Part BT verification: remote `main` was reread after green CI; the only bot
