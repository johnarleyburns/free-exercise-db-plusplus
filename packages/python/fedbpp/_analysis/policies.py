...
def completed_exercise_sets(exercise: dict[str, Any], *, include_types: set[str] | None = None) -> list[dict[str, Any]]:
    """Return completed set observations under the shared ACTUAL policy."""
    return [item for item in exercise.get("sets", []) if item.get("completed", False) and (include_types is None or item.get("setType") in include_types)]


__all__ = ["completed_exercise_sets", "completed_set_count", "planned_set_count"]
