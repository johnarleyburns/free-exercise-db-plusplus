...
def resolve_intent(intent: Any, db: Any, profile: Any = None, target: Any = None, relationships: Any = None, history: Any = None, *, as_of: str | None = None) -> dict[str, Any]:
    ...
    if gp["goal"] != intent["goal"]:
        return {"status": "invalid", "resolvedProfile": None, "resolvedTarget": None, "planningPolicy": None, "goalPolicy": None, "environmentPolicy": None, "generationOptions": {}, "missingInformation": [], "warnings": [], "conflicts": [{"code": "GOAL_POLICY_MISMATCH", "goal": intent["goal"], "requestedGoalPolicy": goal_policy_id, "policyGoal": gp["goal"]}], "defaultsApplied": [], "explicitOverrides": _empty_overrides(), "provenance": {"intentSchemaVersion": intent["schemaVersion"]}}
    ...
    if conflicts or validate_training_profile(profile, db, relationships):
        ...
        return {"status": "invalid", "resolvedProfile": profile, "resolvedTarget": resolved_target, "planningPolicy": policy_id, "goalPolicy": {"policyId": goal_policy_id, "policyVersion": gp["policyVersion"]}, "environmentPolicy": env_id, "generationOptions": generation_options, "missingInformation": [], "warnings": warnings, "conflicts": conflicts, "defaultsApplied": [], "explicitOverrides": _empty_overrides(), "provenance": {"intentSchemaVersion": intent["schemaVersion"]}}
    ...
    explicit_overrides = _empty_overrides()
    explicit_overrides.update({"goalPolicy": bool(intent.get("requestedGoalPolicy")), "planningPolicy": bool(intent.get("requestedPlanningPolicy")), "target": target is not None, "trainingProfile": supplied_profile, "equipmentAdded": sorted(additions), "equipmentRemoved": sorted(removals)})
